    ...
    `}function Q(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}function Ae(e,t){return typeof e=="function"&&e(),typeof t=="function"?t():void 0}var ie=2,ft=10;function yt(e){return(e.protocol==="https:"?"wss:":"ws:")+"//"+e.host+"/beatify/ws"}function pt(e){return Math.min(1e3*Math.pow(2,e-1),3e4)}var y={debug:()=>{},getCurrentGame:()=>null,getCurrentView:()=>null,setIsPlaying:()=>{},setAdminPlayerName:()=>{},setAdminSessionId:()=>{},getAdminPlayerName:()=>null,handleAdminStateUpdate:()=>{},startLobbyPolling:()=>{},stopLobbyPolling:()=>{},showError:()=>{},resetHomeStartButton:()=>{}};function je(e){y={...y,...e||{}}}var p=null,q=0,se=!1,Y=0,Se=!1,oe=!1;function E(){return!!p&&p.readyState===WebSocket.OPEN}function H(e){return E()?(e&&e.action==="start_game"&&(oe=!0),p.send(JSON.stringify(e)),!0):!1}function Oe(){p&&(p.close(),p=null)}function Ue(){q=0}async function M(){if(!Se&&!(p&&(p.readyState===WebSocket.OPEN||p.readyState===WebSocket.CONNECTING))){Se=!0;try{var e=await BeatifyAuth.getAccessToken();if(!e&&!BeatifyAuth.isCompanionBypassMode()||p&&(p.readyState===WebSocket.OPEN||p.readyState===WebSocket.CONNECTING))return;var t=yt(window.location);try{p=new WebSocket(t)}catch(i){console.error("[Admin WS] Failed to create WebSocket:",i);return}var n=p;n.onopen=function(){n===p&&(y.debug("[Admin WS] Connected, sending admin_connect (token: len="+(e?e.length:0)+", prefix="+(e?e.slice(0,12):"null")+", recoveryAttempt="+Y+"/"+ie+")"),q=0,n.send(JSON.stringify({type:"admin_connect",ha_token:e})))},n.onmessage=function(i){if(n===p)try{var s=JSON.parse(i.data);gt(s)}catch(d){console.error("[Admin WS] Message parse error:",d)}},n.onclose=function(){if(n===p&&(y.debug("[Admin WS] Disconnected"),p=null,y.getCurrentView()==="lobby"&&y.startLobbyPolling(),!se&&q<ft&&y.getCurrentGame())){q++;var i=pt(q);setTimeout(M,i)}},n.onerror=function(i){n===p&&console.error("[Admin WS] Error:",i)}}finally{Se=!1}}}function gt(e){switch(e.type){case"admin_connect_ack":y.debug("[Admin WS] Authenticated, game_id:",e.game_id),Y=0,y.stopLobbyPolling();break;case"state":oe=!1,y.handleAdminStateUpdate(e);break;case"join_ack":if(y.setIsPlaying(!0),e.session_id){y.setAdminSessionId(e.session_id);var t=location.protocol==="https:"?"; Secure":"";document.cookie="beatify_session="+e.session_id+"; path=/beatify; max-age=86400; SameSite=Strict"+t}y.debug("[Admin WS] Joined as player:",y.getAdminPlayerName());break;case"metadata_update":if(e.song){var n=document.getElementById("admin-album-art");n&&e.song.album_art&&(n.src=e.song.album_art)}break;case"admin_token_update":z(e.admin_token,e.game_id),y.debug("[Admin WS] Admin token updated for game:",e.game_id);break;case"error":if(console.error("[Admin WS] Error:",e.code,e.message),e.code==="UNAUTHORIZED"){if(se)break;if(Y>=ie){if(BeatifyAuth.isCompanionBypassMode()){console.warn("[Admin WS] Auth recovery exhausted in Companion bypass mode \u2014 Beatify was reached over a non-local address; OAuth re-login would hit the Invalid-redirect-URI screen. Surfacing local-network hint.");var i=window.BeatifyI18n&&BeatifyI18n.t("admin.companionLocalNetworkRequired")||"Beatify admin requires local network access from the Companion app \u2014 open Beatify in an external browser, or connect to your home Wi-Fi.";try{y.showError(i)}catch{}break}console.warn("[Admin WS] Auth recovery exhausted after "+ie+" attempts; HA rejected every bridge-supplied token. Forcing re-login.");var s=window.BeatifyI18n&&BeatifyI18n.t("admin.wsAuthFailed")||"Home Assistant rejected the access token. Re-authenticating\u2026";try{y.showError(s)}catch{}BeatifyAuth.logout(),BeatifyAuth.login();break}se=!0,Y++,console.warn("[Admin WS] UNAUTHORIZED \u2014 recovery attempt "+Y+"/"+ie+" (server message: "+(e.message||"")+")");var d=p;p=null;try{d?.close()}catch{}BeatifyAuth.handleServerRejection().then(function(o){se=!1,o&&(q=0,M())})}else if(e.code==="NAME_TAKEN"||e.code==="NAME_INVALID"){y.showError(e.message),y.setIsPlaying(!1),y.setAdminPlayerName(null);var l=document.getElementById("admin-join-btn");l&&(l.disabled=!1,l.textContent=BeatifyI18n.t("admin.join"))}else oe?(oe=!1,y.resetHomeStartButton(),y.showError(e.message)):console.warn("[Admin WS] Command error:",e.code,e.message);break;default:break}}function N(e){return E()?(p.send(JSON.stringify(e)),!0):(y.showError(BeatifyI18n.t("admin.connectionLost")||"Connection lost \u2014 reconnecting..."),q=0,M(),!1)}function le(e){let t=typeof window<"u"?window:globalThis,n=t&&t.BeatifyUtils;return n&&typeof n.escapeHtml=="function"?n.escapeHtml(e):e==null?"":String(e)}function re(e,t){let n=typeof window<"u"?window:globalThis,i=n&&n.BeatifyI18n;return i&&typeof i.t=="function"?i.t(e,t):e}function Fe(e){var t=document.getElementById("admin-submitted-players");!t||!e||(t.innerHTML=e.map(function(n){var i=(n.name||"?").split(/\s+/).map(function(l){return l[0]}).join("").substring(0,2).toUpperCase(),s=["player-indicator",n.submitted?"is-submitted":"",n.connected===!1?"player-indicator--disconnected":""].filter(Boolean).join(" "),d="";return n.steal_used&&(d+='<span class="player-badge player-badge--steal">\u{1F977}</span>'),n.bet&&(d+='<span class="player-badge player-badge--bet">\u{1F3B2}</span>'),'<div class="'+s+'">'+d+'<div class="player-avatar"><span class="player-initials">'+le(i)+'</span></div><span class="player-name">'+le(n.name)+"</span></div>"}).join(""))}function de(e,t){var n=t?[t]:["admin-playing-leaderboard-list","admin-reveal-leaderboard"];if(e){var i="";e.forEach(function(s){var d=s.rank<=3?"is-top-"+s.rank:"",l=s.connected===!1?"leaderboard-entry--disconnected":"",o=s.connected===!1?'<span class="away-badge">(away)</span>':"",r="";if(s.streak>=2){var c=s.streak>=5?"streak-indicator--hot":"";r='<span class="streak-indicator '+c+'">\u{1F525}'+s.streak+"</span>"}var m="";s.rank_change>0?m='<span class="rank-up">\u25B2'+s.rank_change+"</span>":s.rank_change<0&&(m='<span class="rank-down">\u25BC'+Math.abs(s.rank_change)+"</span>"),i+='<div class="leaderboard-entry '+d+" "+l+'"><span class="entry-rank">#'+s.rank+'</span><span class="entry-name">'+le(s.name)+o+'</span><span class="entry-meta">'+r+m+'</span><span class="entry-score">'+s.score+"</span></div>"}),n.forEach(function(s){var d=document.getElementById(s);d&&(d.innerHTML=i)}),e.length>0&&["admin-playing-leaderboard-summary","admin-reveal-leaderboard-summary"].forEach(function(s){var d=document.getElementById(s);d&&(d.textContent=e[0].name+" \u2014 "+e[0].score)})}}function Ve(e,t,n){var i=document.getElementById("admin-reveal-guesses");if(i){if(!e||e.length===0){i.innerHTML="";return}var s=null;t&&e.forEach(function(o){!o.missed_round&&o.years_off!=null&&(s===null||o.years_off<s)&&(s=o.years_off)});var d=e.slice().sort(function(o,r){return(r.round_score||0)-(o.round_score||0)}),l='<div class="results-cards-scroll">';d.forEach(function(o){var r=o.missed_round===!0,c=o.years_off||0,m=o.round_score||0,u=r?"is-score-zero":m>=10?"is-score-high":m>=1?"is-score-medium":"is-score-zero",g=t&&!r&&s!==null&&c===s,v=g?" is-closest-winner":"",h=r?"\u2014":o.guess||"n/a",f=r?re("reveal.noGuessShort")||"Missed":c===0?re("reveal.exact")||"Exact!":re("reveal.shortOff",{years:c})||c+" off",b=o.bet?'<span class="card-bet">\u{1F3B2}</span>':"",w=g?'<span class="closest-winner-badge">\u{1F3AF}</span>':"",x=o.artist_bonus>0?'<span class="player-card-artist-badge">\u{1F3A4} +'+o.artist_bonus+"</span>":"";l+='<div class="result-card '+u+v+'"><div class="card-name">'+le(o.name)+b+w+'</div><div class="card-guess">'+h+'</div><div class="card-accuracy">'+f+'</div><div class="card-score">+'+m+x+"</div></div>"}),l+="</div>",i.innerHTML=l}}function Qe(e){if(!e)return"";var t={spotify:"admin.pauseRecovery.providerSpotify",apple_music:"admin.pauseRecovery.providerAppleMusic",youtube_music:"admin.pauseRecovery.providerYouTubeMusic",tidal:"admin.pauseRecovery.providerTidal",deezer:"admin.pauseRecovery.providerDeezer"},n={spotify:"Spotify",apple_music:"Apple Music",youtube_music:"YouTube Music",tidal:"Tidal",deezer:"Deezer"},i=t[e];return i&&(re(i)||n[e])||""}var A=window.BeatifyUtils||{};function D(e,t,n=!1){let i=document.getElementById("playlists-list");i?.removeAttribute("data-i18n"),i?.removeAttribute("aria-busy"),i?.classList.remove("skeleton-list");let s=n?[...a.selectedPlaylists]:[];a.selectedPlaylists=[],a.playlistData=e||[],vt(a.playlistData);let d=a.playlistData;!a.activeFilterTags.includes("all")&&a.activeFilterTags.length>0&&(d=a.playlistData.filter(o=>{let r=o.tags||[];return a.activeFilterTags.every(c=>r.includes(c))}));let l=a.playlistData.some(o=>o.is_valid);if(!a.playlistData||a.playlistData.length===0){let o=a.playlistDocsUrl?`<a href="${A.escapeHtml(a.playlistDocsUrl)}" target="_blank" rel="noopener">How to create playlists</a>`:"";i.innerHTML=`
        ...
        `;return}if(i.innerHTML=d.map(o=>{if(o.is_valid){let r=o.song_count||0,c=o.spotify_count||0,m=o.apple_music_count||0,u=o.youtube_music_count||0,g=o.tidal_count||0,v=o.deezer_count||0,h=o.amazon_music_count||r,f=r;a.selectedProvider==="spotify"?f=c||r:a.selectedProvider==="apple_music"?f=m:a.selectedProvider==="youtube_music"?f=u:a.selectedProvider==="tidal"?f=g:a.selectedProvider==="deezer"?f=v:a.selectedProvider==="amazon_music"&&(f=h);let b=f===0,w=b?"is-disabled":"",x=b?"disabled":"",X="";return f<r&&(X=`<span class="${f===0?"playlist-coverage playlist-coverage--none":"playlist-coverage playlist-coverage--warning"}">${f}/${r}</span>`),`
                <div class="playlist-item list-item ${b?"":"is-selectable"} ${w}"
                    ...
                    <span class="meta">${X||A.escapeHtml(String(r))} songs</span>
                </div>
        ...
        `),t.innerHTML=s,t.classList.remove("hidden"),t.querySelectorAll(".filter-dropdown").forEach(l=>{l.addEventListener("change",function(){ht(this.dataset.category,this.value)})})}function ht(e,t){a.activeFilters[e]=t,bt(),D(a.playlistData,"",!0)}function bt(){let e=Object.values(a.activeFilters).filter(t=>t);a.activeFilterTags=e.length>0?e:["all"]}function Je(){a.activeFilters={decade:"",style:"",region:"",special:""},a.activeFilterTags=["all"],D(a.playlistData,"",!0)}function Et(){return a.selectedPlaylists.reduce((e,t)=>e+t.songCount,0)}function Ke(){let e=document.getElementById("playlist-summary"),t=document.getElementById("selected-count"),n=document.getElementById("total-songs");!e||!t||!n||(a.selectedPlaylists.length===0?e.classList.add("hidden"):(e.classList.remove("hidden"),t.textContent=a.selectedPlaylists.length,n.textContent=Et()))}function $(){let e=document.getElementById("start-game"),t=document.getElementById("playlist-validation-msg"),n=document.getElementById("media-player-validation-msg");if(!e)return;let i=a.selectedPlaylists.length===0,s=a.selectedMediaPlayer===null;e.disabled=i||s,t&&t.classList.toggle("hidden",!i),n&&n.classList.toggle("hidden",!s)}var B=window.BeatifyUtils||{};function Bt(e){let t=document.getElementById("media-player-summary");t&&(t.textContent=e||"Select...")}function Xe(e){let t=document.getElementById("media-players-list");t?.removeAttribute("data-i18n"),t?.removeAttribute("aria-busy"),t?.classList.remove("skeleton-list");let n=e?e.length:0;a.selectedMediaPlayer=null;let i=(e||[]).filter(l=>l.state!=="unavailable"),s=document.getElementById("media-player-validation-msg");if(n===0){t.innerHTML=`
        ...
        `,s&&s.classList.add("hidden");return}t.innerHTML=i.map(l=>Lt(l)).join(""),wt();let d=localStorage.getItem(T);if(d){let l=t.querySelector(`.media-player-radio[data-entity-id="${CSS.escape(d)}"]`);if(l){l.checked=!0,J(l,!0);let o=document.getElementById("media-players");if(o){o.classList.add("collapsed");let r=document.getElementById("media-players-toggle");r&&r.setAttribute("aria-expanded","false")}}}}function Lt(e){let t=Ne[e.platform]||{icon:"\u{1F508}",label:e.platform},n=`<span class="platform-badge platform-badge--${B.escapeHtml(e.platform)}">${t.icon} ${t.label}</span>`;return`

...
class ScoringService:
    ...
    def calculate_superlatives(
        ...
        *,
        ...
        if speed_candidates:
            ...
            awards.append({"id": "speed_demon", "emoji": "\u26a1", "title": "speed_demon",
                           "player_name": fastest[0].name, "value": round(fastest[1], 1), "value_label": "avg_time"})

        streak_candidates = [(p, p.best_streak) for p in players if p.best_streak >= MIN_STREAK_FOR_AWARD]
        if streak_candidates:
            best = max(streak_candidates, key=lambda x: x[1])
            awards.append({"id": "lucky_streak", "emoji": "\U0001f525", "title": "lucky_streak",
                           "player_name": best[0].name, "value": best[1], "value_label": "streak"})

        bet_candidates = [(p, p.bets_placed) for p in players if p.bets_placed >= MIN_BETS_FOR_AWARD]
        if bet_candidates:
            most_bets = max(bet_candidates, key=lambda x: x[1])
            awards.append({"id": "risk_taker", "emoji": "\U0001f3b2", "title": "risk_taker",
                           "player_name": most_bets[0].name, "value": most_bets[1], "value_label": "bets"})

        if rounds_played >= MIN_ROUNDS_FOR_CLUTCH:
            clutch_candidates = [(p, p.final_three_score) for p in players if len(p.round_scores) >= MIN_ROUNDS_FOR_CLUTCH]
            if clutch_candidates:
                clutch = max(clutch_candidates, key=lambda x: x[1])
                if clutch[1] > 0:
                    awards.append({"id": "clutch_player", "emoji": "\U0001f31f", "title": "clutch_player",
                                   "player_name": clutch[0].name, "value": clutch[1], "value_label": "points"})
        ...
        if close_candidates:
            ...
            awards.append({"id": "close_calls", "emoji": "\U0001f3af", "title": "close_calls",
                           "player_name": closest[0].name, "value": closest[1], "value_label": "close_guesses"})

        if movie_quiz_enabled:
            movie_candidates = [(p, p.movie_bonus_total) for p in players if p.movie_bonus_total >= MIN_MOVIE_WINS_FOR_AWARD]
            if movie_candidates:
                film_buff = max(movie_candidates, key=lambda x: x[1])
                awards.append({"id": "film_buff", "emoji": "\U0001f3ac", "title": "film_buff",
                               "player_name": film_buff[0].name, "value": film_buff[1], "value_label": "movie_bonus"})

        if intro_mode_enabled:
            intro_candidates = [(p, p.intro_speed_bonuses) for p in players if p.intro_speed_bonuses >= MIN_INTRO_BONUSES_FOR_AWARD]
            if intro_candidates:
                intro_master = max(intro_candidates, key=lambda x: x[1])
                awards.append({"id": "intro_master", "emoji": "\U0001f3a7", "title": "intro_master",
                               "player_name": intro_master[0].name, "value": intro_master[1], "value_label": "intro_bonuses"})
        ...
        if rounds_played >= MIN_ROUNDS_FOR_COMEBACK:
            ...
            if comeback_candidates:
                ...
                awards.append({"id": "comeback_king", "emoji": "\U0001f451", "title": "comeback_king",
                               "player_name": comeback[0].name, "value": comeback[1], "value_label": "improvement"})
    ...
    def calculate_round_analytics(
        ...
        all_guesses = sorted(
            [{"name": p.name, "guess": p.current_guess, "years_off": p.years_off or 0} for p in submitted],
        ...
        speed_champion = None
        timed = [p for p in submitted if p.submission_time is not None and round_start_time is not None]
        if timed:
            fastest = min(p.submission_time - round_start_time for p in timed)
            speed_champion = {
                "names": [p.name for p in timed if (p.submission_time - round_start_time) == fastest],
        ...
        return RoundAnalytics(
            all_guesses=all_guesses, average_guess=avg_guess, median_guess=med_guess,
            closest_players=closest, furthest_players=furthest, exact_match_players=exact,
            exact_match_count=len(exact), scored_count=scored, total_submitted=len(submitted),
            accuracy_percentage=accuracy_pct, speed_champion=speed_champion,
            decade_distribution=decade_dist, correct_decade=_get_decade_label(correct_year),
        )

    @staticmethod
    def score_player_round(
        player: PlayerSession, *, correct_year: int, round_start_time: float | None,
        round_duration: float, difficulty: str, artist_challenge: Any | None,
        movie_challenge: Any | None, is_intro_round: bool,
        intro_round_start_time: float | None, all_players: list[PlayerSession],
        streak_achievements: dict[str, int], bet_tracking: dict[str, int],
    ) -> None:
        """Score a single player for the current round. Mutates player in-place."""
        if player.submitted and correct_year is not None:
            elapsed = (player.submission_time - round_start_time
            ...
            if is_intro_round and intro_round_start_time:
                ...
                if player.submission_time and player.submission_time < cutoff:
                    ...
                    rank = len([p for p in all_players
                                if p.submission_time is not None and p.submission_time < cutoff

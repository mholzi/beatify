...
# =============================================================================
# ARTIST GUESS TESTS (Story 10.1)
# =============================================================================


@pytest.mark.unit
class TestArtistGuessFields:
    """Tests for artist guess fields on PlayerSession (Story 10.1)."""

    def test_player_session_has_artist_fields(self, mock_ws):
        """PlayerSession has artist_guess, artist_score, artist_match fields."""
        from custom_components.beatify.game.player import PlayerSession

        player = PlayerSession(name="Tom", ws=mock_ws)

        assert hasattr(player, "artist_guess")
        assert hasattr(player, "artist_score")
        assert hasattr(player, "artist_match")

    def test_artist_fields_default_values(self, mock_ws):
        """Artist fields have correct default values."""
        from custom_components.beatify.game.player import PlayerSession

        player = PlayerSession(name="Tom", ws=mock_ws)

        assert player.artist_guess is None
        assert player.artist_score == 0
        assert player.artist_match is None


@pytest.mark.unit
class TestSubmitGuessWithArtist:
    """Tests for submit_guess with artist parameter (Story 10.1)."""

    def test_submit_guess_with_artist(self, mock_ws):
        """submit_guess accepts and stores artist parameter."""
        from custom_components.beatify.game.player import PlayerSession

        player = PlayerSession(name="Tom", ws=mock_ws)
        player.submit_guess(year=1985, timestamp=1000.0, artist="Prince")

        assert player.submitted is True
        assert player.current_guess == 1985
        assert player.submission_time == 1000.0
        assert player.artist_guess == "Prince"

    def test_submit_guess_artist_trimmed(self, mock_ws):
        """submit_guess trims artist whitespace."""
        from custom_components.beatify.game.player import PlayerSession
    ...
    def test_submit_guess_without_artist(self, mock_ws):
        """submit_guess works without artist (backwards compatible)."""
        from custom_components.beatify.game.player import PlayerSession
    ...
    def test_submit_guess_empty_artist(self, mock_ws):
        """submit_guess with empty string sets artist_guess to None."""
        from custom_components.beatify.game.player import PlayerSession

        player = PlayerSession(name="Tom", ws=mock_ws)
        player.submit_guess(year=1985, timestamp=1000.0, artist="")

        assert player.artist_guess is None

    def test_submit_guess_whitespace_only_artist(self, mock_ws):
        """submit_guess with whitespace-only string sets artist_guess to None."""
        from custom_components.beatify.game.player import PlayerSession
...
@pytest.mark.unit
class TestResetRoundClearsArtist:
    """Tests for reset_round clearing artist fields (Story 10.1)."""

    def test_reset_round_clears_artist_fields(self, mock_ws):
        """reset_round clears all artist-related fields."""
        from custom_components.beatify.game.player import PlayerSession

        player = PlayerSession(name="Tom", ws=mock_ws)
        # Set artist fields
        player.artist_guess = "Prince"
        player.artist_score = 10
        player.artist_match = "exact"
    ...
    def test_reset_round_clears_artist_after_submission(self, mock_ws):
        """reset_round clears artist fields after full submission."""
        from custom_components.beatify.game.player import PlayerSession

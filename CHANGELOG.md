...
### Added
- **Crate Digger: play from your own music library (#45, #2082).** Contributed by **@DrMagicWolf** — the largest external contribution the project has had, and the answer to a request that had been open since before the tracker was tidied. Games can now be sampled from the host's own Music Assistant library (Plex, Jellyfin, local files) instead of a streaming provider. Playback was never the hard part; MA already plays a library track. The hard part was **metadata good enough to guess against**: local tags carry the *pressing* year, so compilation and greatest-hits rips make whole eras look like the nineties; a difficulty slider is meaningless if "famous" means famous inside one collection rather than in the world; and MA's list models return no genres at all (measured at 20,000 list fetches → zero). The mode therefore builds an enriched, cached pool of the library, verified against MusicBrainz, rather than reading the tags and hoping. Six provider-neutral fixes travelled with it and benefit every user, whether or not they ever switch the mode on.

### Fixed
- **The reveal standings no longer cut names off silently (#2130).** Reported by **@FurtiveD**. Two causes, one symptom: the standings card sat at `overflow: hidden`, so anything past the bottom edge was simply gone — no scrollbar, no hint that there was more — and the lower-third band had no height cap, growing with close-call cards and verdict chips in exactly the rounds where the standings matter most. The card scrolls now, and the band is capped.
- **The reveal standings share their height instead of relying on a scrollbar (#2134).** A scrollbar is not operable on a television with no pointing device, so "scrollable, but nobody can scroll" was the honest version of the same problem rather than its fix. The #963 treatment from the Playing view now applies here too: rows share the card's height, each row is a size container, and the fonts scale as a fraction of the row's own height, so the text fits at any player count and any viewport height. The reason this never arrived with #963 is worth recording — the reveal row had no sizes of its own at all, it inherited everything from the generic leaderboard block. Nothing was forgotten; nothing was ever added.
...
### Changed
- **Music Assistant is set up before Beatify (#2144).** `music_assistant` joins `after_dependencies` in the manifest. On its own this changes almost nothing — Beatify talks to MA through `hass.services.async_call` and `hass.config_entries`, never through an import, so hassfest had nothing to complain about. It is the correct order for an integration that calls another one's services, and it is what the Crate Digger library client needs, because that one *does* import from `homeassistant.components.music_assistant`.
- **The backfill agents stop re-asking for answers they already have (#2150).** The Tidal backfill has always remembered which tracks a provider demonstrably does not carry; Apple, Deezer and YouTube had no such memory and re-queried every open song on every run. Measured on 13 August: of Tidal's 143 remaining gaps, 126 were confirmed misses — exactly the kind of "no" that does not need fetching again. All four agents share one Odesli quota, and that quota returned `429` repeatedly that morning, so the waste was not free.
- **The Apple backfill hears a second opinion before rejecting on year (#2116).** When the year gate discards a candidate, track duration is now consulted: a match within two seconds is accepted after all. The duration only ever overrules in one direction — what the year accepts is never retroactively thrown out, and a test pins that. The two-second line comes from measurement, not taste: of twenty analysable rejections, fourteen sat within two seconds (eight of them at exactly zero), and the hit group ends at 920 ms.
- **Dead reveal CSS removed (#2132).** Every rule mentioning `.reveal-right-section` or `.reveal-left-section` is gone; the classes are left over from the old two-column reveal and no HTML, JS or Python file has assigned them since the switch to `.reveal-broadcast`. One of them reads as though it hides the rank-change arrow in the standings — it does not, the arrow is visible, and it was one of the things that made the name column look guilty during the #2130 analysis.

## [4.2.1-rc1] - 2026-08-11

### Added
- **The host picks how many rounds a game runs (#1475).** Requested by **@Vanman777** on 15 June — the only external feature request in the tracker, and open for 57 days. A game played every song of every selected playlist, so a 100-song playlist meant a two-hour evening nobody asked for; the round count was derived from the playable pool, never set. Step 4 of the setup wizard now offers 10 / 20 / 30 / all / a typed number, with a live duration estimate, and a rematch replays at the chosen length. `max_rounds=0` means "play everything" and is the default, so a client that never sends the field behaves exactly as before. Two placement decisions are pinned by tests: the cap runs *before* the difficulty ramp-up is built (truncating afterwards would serve only the easiest songs), and it samples rather than slices (the pool is grouped by playlist, so `[:n]` would serve only the first of a multi-playlist selection).
- **Sudden Death names the ending it actually had (#2105).** When the rounds run out with two or more players still alive, nobody is the last one standing — `compute_winners` has crowned the top-scoring survivor since #1749, but the END screen had no words for it and fell back to the ordinary podium. It now shows "Best of the Survivors" with the count that explains it, while a genuine 1v1 finish keeps the trophy and "Last One Standing".

### Fixed
- **Minified CSS was outside the drift guard (#2098).** `admin.html` and `player.html` load `styles.min.css`, but the build only ever generated and verified the `.min.js` bundles — the CSS was hand-maintained and drifted silently. **31 class rules and one `@keyframes` never reached the file the browser loads**, spanning three shipped features: the Sudden Death elimination view (merged 5 July), the Streak-Shield badge (22 July) and the Mix-tab continue button (28 June). `.eliminated-view` carries the flex centring and `min-height: 60vh` of the "you're out" screen, so it rendered as unstyled top-aligned text. This is the same failure #1263 had on the JS side, which is why the guard exists; it just never covered CSS. `build:check` now fails on CSS drift too.
- **"Last One Standing" was announced while others were still standing (#2103).** The END screen took the first surviving player whenever Sudden Death was on, without checking how many were still alive — contradicting both the withheld backend award and the leaderboard directly below it. Reachable in practice since the selectable round count: elimination starts in round 2 and removes one player per round, so N players need N rounds, and a cap of 10 leaves 11 players with two survivors.
